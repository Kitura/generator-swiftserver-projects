...
function killServerProcesses() {
    pid="$(pgrep Generator-Swiftserver-Projects)"
    if [ "$pid" != "" ]
    then
        echo "killing Generator-Swiftserver-Projects"
        kill $pid
    fi
